  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#1A1A1A]">
      ...
      <header className="relative w-full h-64 overflow-hidden">
        ...
        <div className="relative z-20 flex justify-between items-start p-4">
          <div className="flex flex-col">
            <h1 className="text-3xl font-bold tracking-wider text-white" style={{ fontFamily: "monospace" }}>
              GARAGE BET
            </h1>
            <p className="text-gray-300 text-sm">Gestion de flotte militaire</p>
          </div>
          <Button variant="ghost" size="icon" className="text-white" onClick={() => router.push("/parametres")}>
      ...
      <main className="flex-1 px-4 py-6 -mt-16 relative z-20">
          ...
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div className="bg-[#2A2A2A] p-3 rounded-lg">
                ...
                <p className="text-2xl font-bold text-green-500">{operationalVehicles}</p>
              </div>
            </div>
            ...
            <div className="mb-2">
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-gray-300">
                  {vehicles.length > 0 ? Math.round((operationalVehicles / vehicles.length) * 100) : 0}%
                </span>
              </div>
              <Progress
                value={vehicles.length > 0 ? (operationalVehicles / vehicles.length) * 100 : 0}
            ...
            <div className="mt-4">
              ...
              <p className="text-2xl font-bold text-red-500">{nonOperationalVehicles}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-[#212121] border-[#333333] shadow-lg mb-4">
          <CardContent className="p-4">
            <h2 className="text-lg font-semibold text-gray-200 mb-3">Dépenses totales</h2>
        ...
        <Card className="bg-[#212121] border-[#333333] shadow-lg">
          <CardContent className="p-4">
            <h2 className="text-lg font-semibold text-gray-200 mb-3">Dernières interventions</h2>
                ...
                {recentInterventions.map((intervention, index) => (
                  <div key={index} className="bg-[#2A2A2A] p-3 rounded-lg">
                    <div className="flex justify-between">
                      <p className="text-gray-300 font-medium">{intervention.vehicleName}</p>
                      <p className="text-[#84cc16]">{Number.parseFloat(intervention.cost).toFixed(2)} €</p>
                    </div>
                    <p className="text-sm text-gray-400">{intervention.date}</p>
                    <p className="text-sm text-gray-400 truncate">{intervention.description}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-gray-400 text-center py-4">Aucune intervention enregistrée</p>
            )}
          </CardContent>
        </Card>
      </main>
